<?php
/**
 * Modified version of HTML_Template_IT 1.2.1 stable
 * Mod version: 0.2
 * 
 * Copyright (c) 2009-2011 Alex® the Rebel™
 *                         http://www.aleksandr.ru
 * 
 * Mod v. 0.1 features:
 * - does't requires PAER package anymore, all in one file now
 * In this version i've added some funnctions to avoid PEAR.
 * 
 * Mod v. 0.2 features:
 * - special template symbols '{', '}', '$' are escaped in setVariable
 * with backslash + html code and unescaped in parse.
 * In this version i've added function to escape and unescape strings.
 */
...
class HTML_Template_IT
{
    ...
    /**
     * Mod 0.1 feature
     * to avoid PEAR::isError
     *
     */
	function isError($error)
	{
		return $error != IT_OK ? true : false;
	}
    
    /**
     * Mod 0.2 feature
     * to avoid deleting '{', '}', '$' symbols in variables
     * 
     * @param string $data
     * @return escaped
     */
    static function escape($data, $unescape = false)
    {
        $search = array('{', '}', '$');
        $replace = array('\&#0123;', '\&#0125;', '\&#0036;');
        return $unescape ? str_replace($replace, $search, $data) : str_replace($search, $replace, $data);
    }
    ...
    function setVariable($variable, $value = '')
    {
        if (is_array($variable)) {
            /* mod 0.2 <--- */
            foreach($variable as $k=>$v) $variable[$k] = $this->escape($v);
            /* end of mod */
            ...
            */
            $this->variableCache[$variable] = $this->escape($value);
        }
    ...
    function errorMessage($value, $blockname = '')
    {
        ...
        if (!isset($errorMessages)) {
            $errorMessages = array(
                ...
                IT_BLOCK_DUPLICATE          => 'The name of a block must be unique within a template. Found "' . $blockname . '" twice. '.
                                               'Unpredictable results may appear.',
